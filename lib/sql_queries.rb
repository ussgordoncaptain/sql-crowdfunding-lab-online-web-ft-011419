...
"SELECT projects.title, SUM(amount) FROM pledges JOIN projects ON pledges.project_id = projects.id GROUP BY projects.title;"
end

def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
"SELECT name, age, SUM( pledges.amount ) FROM users JOIN pledges ON pledges.user_id = users.id GROUP BY users.name; "
end
